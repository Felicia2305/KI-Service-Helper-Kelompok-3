...
class TicketService:
    ...
    async def submit(
        ...
        ticket = self._ticket_repo.update_signature(
            ticket,
            signature.hex()
        )
    ...
    async def claim_specific(self, staff: User, ticket_id: UUID) -> Ticket:
        ...
        if ticket.status != "dalam_antrean":
            raise HTTPException(
                status_code=409,
                detail="Tiket sudah diambil atau tidak lagi dalam antrean."
            )

        staff_level = _STAFF_LEVEL_MAP.get(staff.role)
        if not staff_level:
            raise HTTPException(status_code=403, detail="Role staff tidak dikenali.")
        ...
        if ticket.service_type.level != staff_level:
            ...
            raise HTTPException(
                status_code=403,
                detail="Tiket ini tidak sesuai dengan level Anda."
            )
        ...
        audit_logger.info(
            "TICKET_CLAIMED ticket_id=%s staff=%s level=%s",
            ticket.id, staff.email, staff_level
        )
    ...
    async def claim_next(self, staff: User) -> Ticket:
        ...
        audit_logger.info(
            "TICKET_CLAIMED_NEXT ticket_id=%s staff=%s level=%s",
            ticket.id, staff.email, staff_level
        )
    ...
    def get_ticket_detail(self, requester: User, ticket_id: UUID) -> Ticket:
        ...
        try:
            verify_signature(
                public_key,
                ticket_data,
                bytes.fromhex(ticket.signature)
            )
        except Exception:
            raise HTTPException(
                status_code=400,
                detail="Data ticket telah dimodifikasi"
            )
    ...
    def _get_assigned_ticket(self, staff: User, ticket_id: UUID) -> Ticket:
        ...
        if ticket.assigned_to != staff.id:
            ...
            raise HTTPException(status_code=403, detail="Tiket ini bukan milik Anda.")
    ...
    def _require_staff(self, user: User) -> None:
        if user.role == "mahasiswa":
            raise HTTPException(
                status_code=403,
                detail="Hanya staff yang dapat melakukan aksi ini."
            )
    ...
    async def _save_file(self, file: UploadFile, subfolder: str) -> str:
        ...
        if file.content_type != "application/pdf":
            raise HTTPException(status_code=400, detail="Tipe MIME tidak valid.")
